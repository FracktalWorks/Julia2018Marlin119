      ...
      static void babystep_axis(const AxisEnum axis, const int16_t distance) {
        if (axis_known_position[axis]) {
              ...
              babystepsTodo[Z_AXIS] += distance;
							...
							// PRINT RESTORE
							#if ENABLED(PRINT_RESTORE) && ENABLED(BABYSTEPPING)
								babystep_total += ((float) distance) / planner.axis_steps_per_mm[Z_AXIS]; // (offs * planner.axis_steps_per_mm[Z_AXIS]);
								SERIAL_PROTOCOLPAIR("Babystep: ", distance);
								SERIAL_PROTOCOLLNPAIR(" total: ", babystep_total);
							#endif
							/* FRACKTAL WORKS: END */
            #endif
          #else
