...
RestorationPhase restoration_phase = IDLE;
...
void do_print_job_recovery() {
  ...
  //ZERO(job_recovery_commands);
	...
	// SERIAL_PROTOCOLLNPAIR("Ressurect file: ", RECOVERY_FILE_NAME);
  ...
  if (card.cardOK) {
    ...
    if (card.jobRecoveryFileExists()) {
			restoration_phase = BIN_FOUND;
      ...
      // card.removeJobRecoveryFile();
			...
			if (job_recovery_info.valid_head && job_recovery_info.valid_head == job_recovery_info.valid_foot) {
				...
				card.openFile(RECOVERY_FILE_NAME, false);
				...
				sprintf_P(cmd, PSTR("M23 %s"), job_recovery_info.sd_filename);
				card.write_command(cmd);
				...
				/*
				sprintf_P(cmd, PSTR("M32 P S%lu !%s#"), job_recovery_info.sdpos, job_recovery_info.sd_filename);
				card.write_command(cmd);
				*/
				
				card.closefile();
				
				restoration_phase = FILE_MADE;	
...
void save_job_recovery_info() {
  ...
  if (restoration_phase != IDLE) return;
